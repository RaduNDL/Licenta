...
public class InboxModel : PageModel
{
        ...
        bool includeHidden)
    {
        ...
        foreach (var r in requests)
        {
                ...
                r.Status == PatientMessageRequestStatus.Closed)
            {
                ...
                {
                    var doctorConversation = await BuildConversationAsync(
                        patient,
                        r,
                        "Doctor",
                        doctorUserId,
            ...
            {
                var assistantConversation = await BuildConversationAsync(
                    patient,
                    r,
                    "Assistant",
                    r.AssistantId,
    ...
    private async Task<ConversationVm> BuildConversationAsync(
        ApplicationUser patient,
        ...
        HashSet<string> hiddenConversationKeys)
    {
        var partner = await _db.Users
            .AsNoTracking()
            .FirstOrDefaultAsync(u => u.Id == partnerUserId);

        var lastMsg = await _db.InternalMessages
            .AsNoTracking()
            .Where(m =>
                m.Subject == ThreadKey(req.Id) &&
                ((m.SenderId == patient.Id && m.RecipientId == partnerUserId) ||
                 (m.SenderId == partnerUserId && m.RecipientId == patient.Id)))
            .OrderByDescending(m => m.SentAt)
            .FirstOrDefaultAsync();

        var preview = lastMsg?.Body ?? req.Subject ?? "No messages";
        if (preview.Length > 40)
            preview = preview[..40] + "...";

        var conversationKey = ConversationKey(req.Id, kind);
        ...
        return new ConversationVm
        {
            ...
            LastAt = lastMsg?.SentAt ?? (req.UpdatedAt ?? req.CreatedAt),
            ConversationKey = conversationKey,
            IsClosed = isClosed,
            IsHidden = hiddenConversationKeys.Contains(conversationKey)
        };
    }

    private async Task LoadMessagesAsync(ApplicationUser patient, Guid reqId, string partnerUserId)
    {
    ...
    public async Task<IActionResult> OnGetAsync()
    {
        ...
        await LoadMessagesAsync(patient, req.Id, resolvedPartnerId);
