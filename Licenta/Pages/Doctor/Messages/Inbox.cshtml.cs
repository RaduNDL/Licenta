...
public class InboxModel : PageModel
{
    ...
    private const string ConversationVisibilityEntity = "DoctorConversation";
    private const string HideConversationAction = "HideConversationDoctor";
    private const string RevealConversationAction = "RevealConversationDoctor";
        ...
        bool includeHidden)
    {
        ...
        var threadKeys = requests.Select(r => ThreadKey(r.Id)).ToList();

        var lastMessages = await _db.InternalMessages
            .AsNoTracking()
            .Where(m => threadKeys.Contains(m.Subject))
            .GroupBy(m => m.Subject)
            .Select(g => new
            {
                Subject = g.Key,
        ...
        {
            if (req.Patient == null || string.IsNullOrWhiteSpace(req.Patient.UserId))
                continue;

            var threadKey = ThreadKey(req.Id);
            var convKey = ConversationKey(req.Id);
    ...
    public async Task<IActionResult> OnGetAsync()
    {
        ...
        var hiddenConversationKeys = await GetHiddenConversationKeysAsync(doctorUser.Id);
        await LoadConversationsAsync(doctorUser, doctorProfile.Id, hiddenConversationKeys, ShowHidden);
    ...
    public async Task<IActionResult> OnPostHideConversationAsync(Guid requestId)
    {
        ...
        await WriteConversationVisibilityAsync(doctorUser.Id, req.Id, HideConversationAction);
    ...
    public async Task<IActionResult> OnPostRevealConversationAsync(Guid requestId)
    {
        ...
        {
            TempData["StatusMessage"] = "Conversation not found.";
            return RedirectToPage("/Doctor/Messages/Inbox", new { showHidden = true });
        }

        await WriteConversationVisibilityAsync(doctorUser.Id, req.Id, RevealConversationAction);
