...
namespace Licenta.Pages.Patient.Appointments
{
    ...
    public class IndexModel(AppDbContext db, UserManager<ApplicationUser> userManager) : PageModel
    {
        ...
        public async Task OnGetAsync()
        {
            ...
            var appts = await _db.Appointments
                ...
                .Select(x => new AppointmentRowVm
                {
                    ...
                    DoctorName = x.Doctor != null ? (x.Doctor.User != null ? (x.Doctor.User.FullName ?? x.Doctor.User.Email ?? "-") : "-") : "-",
        ...
        private static bool HasMarker(ParsedNotes parsed, string? rawNotes, string marker)
        {
            ...
            if (!string.IsNullOrWhiteSpace(rawNotes) && rawNotes.Contains(marker, StringComparison.OrdinalIgnoreCase))
                return true;
        ...
        private static ParsedNotes ParseValidationNotes(string? notes)
        {
            ...
            if (first.Contains(':'))
            {
                var (key, value) = SplitKeyValue(first);
                if (key.Length > 0 && value.Length > 0)
                    parsed.Parts[key] = value;
            }
            else
            {
        ...
        private static string FormatLocalIsoForDisplay(string localIso)
        {
            ...
            return local.ToString("ddd dd MMM HH:mm", CultureInfo.InvariantCulture);
        }
    }
}
