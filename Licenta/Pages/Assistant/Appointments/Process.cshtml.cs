...
namespace Licenta.Pages.Assistant.Appointments
{
    ...
    public class ProcessModel(
        ...
        INotificationService notifier) : PageModel
    {
        ...
        public List<SlotVm> AvailableSlots { get; set; } = [];
        ...
        public async Task<IActionResult> OnGetAsync(Guid? attachmentId, Guid? doctorId)
        {
            ...
            var assistantClinic = (assistant.ClinicId ?? "").Trim();
            if (!string.IsNullOrWhiteSpace(assistantClinic))
            {
                var patientClinic = (att.Patient?.User?.ClinicId ?? "").Trim();
                if (!string.Equals(patientClinic, assistantClinic, StringComparison.Ordinal))
                    return Forbid();
            }
        ...
        private async Task<IActionResult> HandleAsync(bool createImmediately)
        {
            ...
            if (!string.IsNullOrWhiteSpace(assistantClinic))
            {
                var doctorClinic = (doctorEntity.User?.ClinicId ?? "").Trim();
                if (!string.Equals(doctorClinic, assistantClinic, StringComparison.Ordinal))
                    return Forbid();
            }
            ...
            if (daySlots.Count == 0 || !daySlots.Exists(s => time >= s.StartTime && time < s.EndTime))
            {
        ...
        private async Task LoadDoctorsAsync(string? clinicId, Guid? preferredDoctorId)
        {
            ...
            if (!string.IsNullOrWhiteSpace(cid))
                doctorsQuery = doctorsQuery.Where(d => d.User != null && (d.User.ClinicId ?? "") == cid);

            var list = await doctorsQuery
                .OrderBy(d => d.User != null ? (d.User.FullName ?? d.User.Email) : "")
                .Select(d => new
                {
                    d.Id,
                    Name = d.User != null ? (d.User.FullName ?? d.User.Email ?? "(no name)") : "(no user)"
                })
                .ToListAsync();
        ...
        private async Task<AppointmentRequestPayload?> TryReadPayloadAsync(string? filePath)
        {
            ...
            var root = Path.GetFullPath(Path.Combine(_env.ContentRootPath, "Files"));
        ...
        private async Task<List<SlotVm>> BuildSlotsAsync(Guid doctorId, List<DoctorAvailability> schedule)
        {
            ...
            {
                var daySchedule = schedule.Where(s => s.IsActive && s.DayOfWeek == day.DayOfWeek).ToList();
                ...
                foreach (var s in daySchedule)
                {
                    for (var t = s.StartTime; t < s.EndTime; t = t.Add(TimeSpan.FromMinutes(SlotMinutes)))
                    {
                        ...
                        {
                            LocalIso = local.ToString("yyyy-MM-ddTHH:mm", CultureInfo.InvariantCulture),
                            Display = local.ToString("ddd dd MMM HH:mm", CultureInfo.InvariantCulture)
                        });
                    }
                }
            }
            ...
            return result.Count == 0
                ? []
                : result.OrderBy(x => x.LocalIso, StringComparer.Ordinal).ToList();
        }
        ...
        private static bool IsAwaitingDoctorApproval(string? notes)
        {
            ...
            return notes.Contains("AWAITING_DOCTOR_APPROVAL", StringComparison.OrdinalIgnoreCase);
        }
